...
SUPPORTED_LANGS = {
    ...
    "bn": {"name": "Bengali", "ttscode": "bn"},
    "gu": {"name": "Gujarati", "ttscode": "gu"},
    "pa": {"name": "Punjabi", "ttscode": "pa"},
    "or": {"name": "Odia", "ttscode": "or"},
    "ur": {"name": "Urdu", "ttscode": "ur"}
}
...
def transcribe():
    ...
    Returns JSON with paths to generated .vtt (original language transcription) and detected source language.
    """
    try:
        ...
        # Save VTT for the original language
        vtt_name = f"{filename}.original.vtt"
        vtt_path = os.path.join(app.config["UPLOAD_FOLDER"], vtt_name)
        segments_to_vtt(normalized, vtt_path)

        # Return relative URLs so frontend can load them
        vtt_url = url_for("uploaded_file", filename=vtt_name)

        # Get language name
        lang_name = SUPPORTED_LANGS.get(detected_lang, {}).get("name", detected_lang.upper())
        ...
        print("Transcription error:", error_details)
